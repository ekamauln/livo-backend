...
// @Description Get all ribbon flows with pagination and search, primary tracking from mb-ribbon.
// @Tags ribbons
// @Accept json
// @Produce json
// @Security BearerAuth
...
func (rfc *RibbonFlowController) buildRibbonFlow(tracking string) RibbonFlowResponse {
	...
	if err := rfc.DB.Preload("User").Where("tracking = ?", tracking).First(&qcRibbon).Error; err == nil {
		var operator *RibbonUserFlowInfo
		if qcRibbon.QcOperator != nil {
			operator = &RibbonUserFlowInfo{
...
type QcRibbonFlowInfo struct {
	Operator  *RibbonUserFlowInfo `json:"operator,omitempty"`
